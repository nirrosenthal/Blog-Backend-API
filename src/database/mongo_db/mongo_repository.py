...
class MongoDBRepository(Repository):
    ...
    def __init__(self):
        ...
        self._messages_collection = self._messages_db["messages"]
        self._users_db = self._client["users"]
    ...
    def get_post_blog(self, post_id:str) ->Post:
        post_data = self._messages_collection.find_one({"_id": ObjectId(post_id)})
    ...
    def create_post_blog(self, content:str, user_id_owner: str)->Post:
        ...
        created_post_result = self._messages_collection.insert_one(new_post)
        post_data = self._messages_collection.find_one({"_id": created_post_result.inserted_id})
    ...
    def edit_post_blog(self, post_id: str, new_content:str, user_id_owner: str)->Post:
        ...
        self._messages_collection.update_one({"_id": post_id_obj},
                                           {"$set": {"content": new_content}})

        edited_post_data = self._messages_collection.find_one({"_id": post_id_obj})
    ...
    def delete_post_blog(self, post_id:str, user_id_owner: str)->Post:
        post_id_obj = ObjectId(post_id)
        post_to_delete = self._messages_collection.find_one({"_id": post_id_obj})
        reply_comments_to_delete = self._messages_collection.find({"reply_to_message_id": post_id_obj})
        for comment in reply_comments_to_delete:
            self.delete_comment_blog(comment["_id"], comment["user_id_owner"])

        self._messages_collection.delete_one({"_id": post_id_obj})

        return MongoDBRepository._create_post_with_post_data(post_to_delete)


    def get_comment_blog(self, comment_id:str) ->Comment:
        comment_data = self._messages_collection.find_one({"_id": ObjectId(comment_id)})
    ...
    def create_comment_blog(self, content:str, reply_to_message_id:str, user_id_owner:str)->Comment:
        ...
        created_comment_result = self._messages_collection.insert_one(new_comment)
        comment_data = self._messages_collection.find_one({"_id": created_comment_result.inserted_id})
        return MongoDBRepository._create_comment_with_comment_data(comment_data)


    def edit_comment_blog(self, comment_id:str, new_content: str, user_id_owner: str)->Comment:
        comment_id_obj = ObjectId(comment_id)
        self._messages_collection.update_one({"_id": comment_id_obj},
                                           {"$set": {"content": new_content}})

        edited_comment_data = self._messages_collection.find_one({"_id": comment_id_obj})
    ...
    def delete_comment_blog(self, comment_id:str, user_id_owner: str)->Comment:
        comment_id_obj = ObjectId(comment_id)
        comment_to_delete = self._messages_collection.find_one({"_id": comment_id_obj})
        reply_comments_to_delete = self._messages_collection.find({"reply_to_message_id": comment_id_obj})
        for comment in reply_comments_to_delete:
            self.delete_comment_blog(comment["_id"], comment["user_id_owner"])

        self._messages_collection.delete_one({"_id": comment_id_obj})
    ...
    def add_message_like(self, message_id: str, user_id: str) -> bool:
        ...
        message = self._messages_collection.find_one({"_id": message_id_obj})

        if not message or user_id in message.get("user_likes", []):
            return False

        self._messages_collection.update_one(
            {"_id": message_id_obj},
...
if __name__=="__main__":
    ...
    b1 = mongo.add_message_like(first_post.post_id,"second_user")
    updated_post = mongo.get_post_blog(first_post.post_id)
    print(updated_post.user_likes)
    b2 = mongo.remove_message_like(first_post.post_id, "second_user")
    if not (b1 and b2):
    ...
    mongo.get_post_blog(first_post.post_id)